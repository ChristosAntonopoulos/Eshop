# Eshop — LCG Shop Frontend

Fitness / sports supplement storefront (**LCG Shop**, near OAKA / Marousi).

## Projects

| Folder | Description |
|--------|-------------|
| [`lcg-shop-frontend/`](lcg-shop-frontend/) | React + TypeScript + Vite SPA |

## Quick start

```bash
cd lcg-shop-frontend
npm install
npm run dev
```

## Deployment

- **Kubernetes:** NodePort `30083` — see [lcg-shop-frontend/docs/DEPLOYMENT.md](lcg-shop-frontend/docs/DEPLOYMENT.md)
- **CI/CD:** [`azure-pipelines.yml`](azure-pipelines.yml)
- **Backend API contract:** [lcg-shop-frontend/docs/openapi.yaml](lcg-shop-frontend/docs/openapi.yaml)

## License

MIT (or adjust as needed for your shop).
